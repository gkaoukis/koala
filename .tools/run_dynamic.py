...
def get_environment(
    ...
    env = os.environ.copy()
    dynamic_shell = root / ".tools" / "run_dynamic_shell.py"
    io_shell = root / ".tools" / "io_shell.py"
...
if __name__ == "__main__":
    ...
    with tempfile.NamedTemporaryFile() as data_log:
        ...
        mortem_log = (
            root / ".tools" / "target" / "process-logs" / f"{bench_id}.mortem"
        )
        env = get_environment(
            root=root,
        ...
        compressed_data_log = (
            root / ".tools" / "target" / "process-logs" / f"{bench_id}.jsonl.xz"
        )
