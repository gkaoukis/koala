...
- **Submit a pull request** after approval:
  - Structure your benchmark according to Koala’s layout:
    ```
    benchmarks/<your-benchmark-name>/
    ├── scripts/        # One or more .sh files that implement the workload
    ├── install.sh      # Installs necessary software dependencies
    ├── fetch.sh        # Downloads inputs (stored permanently online if possible)
    ├── execute.sh      # Runs all scripts
    ├── validate.sh     # Validates correctness
    └── clean.sh        # Cleans up inputs and outputs
    ```

  - Make sure to test the benchmark under both local (`--bare`) and Docker execution.
  - Include appropriate input sizes (`--min`, `--small`, `--full`) and sample runs if applicable.
...
- **Describe and justify the addition**:
  ...
  - If relevant, cite a paper or system the benchmark represents. For consistency, use the citation format used in the Koala benchmark paper.  
    _Example_:  
    ...
    > **References**
    > - [17] Cappellini, E., Welker, F., Pandolfi, L., et al. (2019). Early Pleistocene enamel proteome from Dmanisi resolves Stephanorhinus phylogeny. *Nature*, 574(7776), 103–107.  
    > - [38] Ibrahim, F., Oppelt, J., Maragkakis, M., & Mourelatos, Z. (2021). TERA-Seq: true end-to-end sequencing of native RNA molecules for transcriptome characterization. *Nucleic Acids Research*, 49(20), e115.  
