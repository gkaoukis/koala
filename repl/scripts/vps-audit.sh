#!/bin/sh
# shellcheck disable=SC2034,SC3037,SC2129,SC2086,SC2126
# All pre-existing, throughout this vendored-style audit script (unused
# TIMESTAMP, echo -e, individual redirects, unquoted vars, grep|wc -l) —
# unrelated to this file's platform-skip check below.
...
if [ "$(uname -s)" != "Linux" ]; then
    echo "vps-audit.sh: skipped, not supported on this platform ($(uname -s)) — reads /proc, /sys, dpkg/apt" >&2
